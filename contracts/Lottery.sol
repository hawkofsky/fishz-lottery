...
contract Lottery is LotteryOwnable, Initializable {
    ...
    // issueId => [totalAmount, firstMatchAmount, secondMatchingAmount, thirdMatchingAmount, burnAmount]
    ...
    function reset() external onlyAdmin {
        ...
        // burn tokens
        uint256 burnAmount = getMatchingRewardAmount(issueIndex - 1, 1);
        fishz.safeTransfer(deadAddress, burnAmount);
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        fishz.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function  multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        fishz.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            fishz.safeTransfer(address(msg.sender), reward);
        }
    ...
    function  multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            fishz.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function calculateMatchingRewardAmount() internal view returns (uint256[5] memory) {
        ...
        uint256 totalAmout4 = totalAmount.sub(totalAmout1).sub(totalAmout2).sub(totalAmout3);
    ...
    function adminWithdraw(uint256 _amount) public onlyAdmin {
        fishz.safeTransfer(address(msg.sender), _amount);
